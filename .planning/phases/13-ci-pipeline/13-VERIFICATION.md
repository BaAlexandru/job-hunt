...
| 4 | Container images are scanned for vulnerabilities and results appear in the workflow summary | VERIFIED | Trivy scans both images with `aquasecurity/trivy-action@0.35.0`; scan output written to files then appended to `$GITHUB_STEP_SUMMARY` via `cat >> $GITHUB_STEP_SUMMARY` (fixed in cbbb852) |
